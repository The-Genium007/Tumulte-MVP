...
describe('usePollInstance Composable', () => {
  ...
  test('fetchPollInstance() should fetch poll instance details', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://localhost:3333/api/v2/mj/polls/poll-instance-123', {
      credentials: 'include',
    })
  ...
  test('fetchPollInstance() should handle network errors', async () => {
    ...
    await expect(fetchPollInstance('poll-instance-123')).rejects.toThrow('Network error')
  })
