...
describe('usePollTemplates Composable', () => {
  ...
  test('fetchTemplates() should load templates list', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://localhost:3333/api/v2/mj/poll-templates', {
      credentials: 'include',
    })
  ...
  test('fetchTemplates() should load templates for specific campaign', async () => {
    const mockTemplates = [createMockPollTemplate({ id: '1', campaignId: 'campaign-123' })]
  ...
  test('updateTemplate() should update existing template', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://localhost:3333/api/v2/mj/poll-templates/1', {
      method: 'PUT',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('launchPoll() should launch poll from template', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://localhost:3333/api/v2/mj/polls/launch', {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('launchPoll() should handle errors', async () => {
    ...
    await expect(launchPoll('template-789')).rejects.toThrow('Failed to launch poll')
  })
})
