...
export const handlers = [
  ...
  http.put(`${API_URL}/api/v2/mj/campaigns/:id`, async ({ params, request }) => {
    const body = (await request.json()) as {
      name?: string
      description?: string
    }
    return HttpResponse.json(createMockCampaign({ id: params.id as string, ...body }))
  }),
  ...
  http.post(`${API_URL}/api/v2/mj/campaigns/:campaignId/poll-templates`, async ({ request }) => {
  ...
  http.post(`${API_URL}/api/v2/mj/campaigns/:campaignId/polls/launch`, async ({ request }) => {
  ...
  http.post(`${API_URL}/api/v2/mj/campaigns/:campaignId/polls/:pollId/cancel`, () => {
    return HttpResponse.json(
      createMockPollInstance({
        status: 'CANCELLED',
  ...
  http.post(`${API_URL}/api/v2/mj/campaigns/:campaignId/invitations`, async ({ request }) => {
