...
export const usePushNotificationsStore = defineStore('pushNotifications', () => {
  ...
  const isPushEnabled = computed(() => preferences.value?.pushEnabled ?? true)
  ...
  const isCurrentBrowserSubscribed = computed(() => {
    ...
    return subscriptions.value.some((s) => s.endpoint === browserEndpoint.value)
  })
  ...
  async function initialize(): Promise<void> {
    ...
    try {
      ...
      await Promise.all([refreshBrowserEndpoint(), fetchSubscriptions(), fetchPreferences()])
  ...
  async function fetchVapidPublicKey(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      console.error('[Push] VAPID key fetch failed:', response.status, errorData)
  ...
  async function fetchSubscriptions(): Promise<void> {
    try {
      ...
      console.log('[Push] fetchSubscriptions() - response status:', response.status)
      if (!response.ok) {
    ...
    try {
      ...
      const registration = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(new Error('Service Worker timeout - vérifiez la validité du certificat SSL')),
            10000
          )
        ),
      ])
      ...
      console.log('[Push] Subscribe complete! Subscriptions count:', subscriptions.value.length)
  ...
  async function deleteSubscription(id: string): Promise<void> {
    try {
      ...
      const subscriptionToDelete = subscriptions.value.find((s) => s.id === id)
  ...
  function checkPermissionStatus(): void {
    ...
    if (typeof localStorage !== 'undefined') {
      bannerDismissed.value = localStorage.getItem('pushNotificationBannerDismissed') === 'true'
    }
  }
