...
export const useVttConnections = () => {
   ...
   */
  const getConnectionDetails = async (id: string): Promise<VttConnectionWithCampaigns> => {
    try {
      const response = await fetch(`${API_URL}/mj/vtt-connections/${id}`, {
        credentials: 'include',
      })
      if (!response.ok) throw new Error('Failed to fetch VTT connection details')
      const data = await response.json()
      return data
    } catch (error) {
  ...
  const regenerateApiKey = async (id: string): Promise<VttConnection> => {
    try {
      const response = await fetch(`${API_URL}/mj/vtt-connections/${id}/regenerate-key`, {
        method: 'POST',
        credentials: 'include',
      })
