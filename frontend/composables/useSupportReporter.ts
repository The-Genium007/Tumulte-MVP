  ...
  return {
    navigation: navigation
      ? {
          ...
          transferSize: (navigation as unknown as { transferSize?: number })?.transferSize,
        }
      : undefined,
    memory: memory
      ? {
...
export const useSupportReporter = () => {
  ...
  const buildStoreSnapshot = () => ({
    auth: {
      ...
      displayName:
        ...
        (authStore.user as unknown as { display_name?: string })?.display_name ?? null,
      email: authStore.user?.email ?? null,
      streamer: authStore.user?.streamer,
    },
    pollControl: {
      activeSessionId: (pollControlStore.activeSession as unknown as { id?: string })?.id ?? null,
  ...
  const sendBugReport = async (
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData?.error || "Impossible d'envoyer le rapport de bug.")
    }
   ...
   */
  const sendSuggestion = async (title: string, description: string): Promise<SuggestionResult> => {
    if (!title || title.trim().length < 5) {
      throw new Error('Le titre doit faire au moins 5 caractères.')
    }
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null)
      throw new Error(errorData?.error || "Impossible d'envoyer la suggestion.")
    }
