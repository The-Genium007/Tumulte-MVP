...
class OfflineStorage {
   ...
   */
  async get<T>(storeName: StoreName, key: string = 'default'): Promise<T | null> {
    try {
      const db = await this.init()
      // Use type assertion for dynamic store access
      const stored = (await db.get(storeName as 'user', key)) as StoredData<T> | undefined
   ...
   */
  async getDataAge(storeName: StoreName, key: string = 'default'): Promise<number | null> {
    try {
      const db = await this.init()
      const stored = (await db.get(storeName as 'user', key)) as StoredData<unknown> | undefined
    ...
    try {
      ...
      for (const storeName of storeNames) {
        ...
        for (const key of keys) {
          const value = (await store.get(key)) as StoredData<unknown> | undefined
 ...
 */
export async function storeCampaignDetail(campaign: CampaignDetail): Promise<void> {
  await offlineStorage.set<CampaignDetail>('campaignDetails', campaign, campaign.id)
}

/**
 * Get stored campaign detail
 */
export async function getStoredCampaignDetail(campaignId: string): Promise<CampaignDetail | null> {
 ...
 */
export async function getStoredPollTemplates(campaignId?: string): Promise<PollTemplate[] | null> {
  const key = campaignId ?? 'global'
 ...
 */
export async function storePolls(campaignId: string, polls: Poll[]): Promise<void> {
 ...
 */
export async function getStoredPolls(campaignId: string): Promise<Poll[] | null> {
...
export async function storePollResults(results: PollResults): Promise<void> {
  await offlineStorage.set<PollResults>('pollResults', results, results.pollInstanceId)
}

/**
 * Get stored poll results
 */
export async function getStoredPollResults(pollInstanceId: string): Promise<PollResults | null> {
 ...
 * Store invitations
 */
export async function storeInvitations(invitations: CampaignInvitation[]): Promise<void> {
  await offlineStorage.set<CampaignInvitation[]>('invitations', invitations, 'pending')
}

/**
 * Get stored invitations
 */
export async function getStoredInvitations(): Promise<CampaignInvitation[] | null> {
