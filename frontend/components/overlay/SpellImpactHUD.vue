...
export interface SpellImpactData {
  instanceId: string
  eventName: string
  actionType: 'spell_disable' | 'spell_buff' | 'spell_debuff'
  success: boolean
  message?: string
  spellName?: string
  spellImg?: string
...
const displayTitle = computed(() => {
  ...
  switch (props.data.actionType) {
    case 'spell_disable':
      return '🔒 SORT BLOQUÉ !'
    case 'spell_buff':
      return '⬆️ SORT AMPLIFIÉ !'
    case 'spell_debuff':
      return '⬇️ SORT MAUDIT !'
    default:
...
const displayDetail = computed(() => {
  if (!props.data) return ''
  const spellName = props.data.spellName ?? ''

  switch (props.data.actionType) {
    case 'spell_disable': {
      const durationMin = props.data.effectDuration
        ? Math.round(props.data.effectDuration / 60)
        : 10
      return `${spellName} (${durationMin} min)`
    }
    case 'spell_buff': {
      const buffLabel =
        props.data.buffType === 'advantage' ? 'Avantage' : `+${props.data.bonusValue ?? 2}`
      return `${spellName} (${buffLabel})`
    }
    case 'spell_debuff': {
      const debuffLabel =
        props.data.debuffType === 'disadvantage'
          ? 'Désavantage'
          : `-${props.data.penaltyValue ?? 2}`
      return `${spellName} (${debuffLabel})`
    }
    default:
      return props.data.message || spellName
  }
})
...
onMounted(() => {
  // Reuse dice-reverse impact sound for now
...
<template>
  <Transition name="spell-impact">
    <div
      v-if="visible && data && animationPhase !== 'idle'"
      class="spell-impact-hud"
    ...
    >
      ...
      >
        ...
        <img v-if="data.spellImg" :src="data.spellImg" class="impact-spell-img" alt="" />
...
.impact-container {
  ...
  box-shadow:
    ...
    0 0 30px color-mix(in srgb, var(--theme-color, #3b82f6) 30%, transparent),
...
.impact-title {
  ...
  text-shadow:
    ...
    0 0 20px color-mix(in srgb, var(--theme-color, #3b82f6) 40%, transparent);
  letter-spacing: 1px;
  margin-bottom: 8px;
  white-space: nowrap;
}
